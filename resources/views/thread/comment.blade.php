            ...
            <div class="row">
                <div class="Threads col-md-8">
                    <div class="main_card card">
                        ...
                        <div class="md-form">
                            <textarea class="md-textarea form-control" id='comment' rows='7' name="comment"></textarea>
                            <label for="comment">コメントを入力！(必須)</label>
                        </div>
                        <input type="hidden" id='th' value={{$detail}}>
                        <div class="text-right">
                            <button id='ajax' class="btn btn-primary">送信</button>
                        </div>
                    </div>
                </div>
                    ...
                    $.each(value.comment,function(k,index){
                        if(!index.hide_setting){
                            ...
                            if($('button[name="'+ new1 +'"]').prop('disabled')==true){
                                re += "<br><button id='high' name='"+ index.com_num +"1' value='"+ index.com_num +"' class='btn btn-primary' disabled='disabled'>"+ index.high_rating +"  高評価</button>";
                            }else{
                                re += "<br><button id='high' name='"+ index.com_num +"1' value='"+ index.com_num +"' class='btn btn-primary'>"+ index.high_rating +"  高評価</button>";
                            }
                            var new2 = ''+index.com_num+'2'
                            if($('button[name="'+ new2 +'"]').prop('disabled')==true){
                                re += "<button id='low' name='"+ index.com_num +"2' value='"+ index.com_num +"' class='btn btn-primary' disabled='disabled'>"+ index.low_rating +"  低評価</button>";
                            }else{
                                re += "<button id='low' name='"+ index.com_num +"2' value='"+ index.com_num +"' class='btn btn-primary'>"+ index.low_rating +"  低評価</button>";
                            }
